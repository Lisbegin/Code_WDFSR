...
class SRFlowNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, scale=4, K=None, opt=None, step=None):
        ...
        self.harr=flow.HaarDownsampling(3)
    ...
    def normal_flow(self, gt, lr, y_onehot=None, epses=None, lr_enc=None, add_gt_noise=True, step=None):
        ...
        # h,_=self.harr(lr_enc['color_map'],0)
        # gh,_=self.harr(gt,0)
        ...
        # fea4=h[:,9:]
        # g4=gh[:,9:]
        # mean1 = squeeze2d(fea1, 4) if random.random() > 0.2    else squeeze2d(
        #         g1/(g1.sum(dim=1, keepdims=True) + 1e-4), 4)
        # m=mean1
        # mean2 = squeeze2d(fea2, 4) if random.random() > 0.2    else squeeze2d(
        #         g2/(g2.sum(dim=1, keepdims=True) + 1e-4), 4)
        # m=torch.cat((m,mean2),dim=1)
        # mean3 = squeeze2d(fea3, 4) if random.random() > 0.2    else squeeze2d(
        #         g3/(g3.sum(dim=1, keepdims=True) + 1e-4), 4)
        # m=torch.cat((m,mean3),dim=1)
        # mean4 = squeeze2d(fea4, 4) if random.random() > 0.2    else squeeze2d(
        #         g4/(g4.sum(dim=1, keepdims=True) + 1e-4), 4)
        # m=torch.cat((m,mean4),dim=1)
    ...
    def rrdbPreprocessing(self, lr):
        ...
        if len(block_idxs) > 0:
            ...
            if opt_get(self.opt, ['network_G', 'flow', 'stackRRDB', 'concat']) or False:
                keys = ['last_lr_fea', 'fea_up1', 'fea_up2', 'fea_up4','fea_up8']
    ...
    def reverse_flow(self, lr, z, y_onehot, eps_std, epses=None, lr_enc=None, add_gt_noise=True):
        ...
        # fea3=h[:,6:9]
        # fea4=h[:,9:]
        # mean1 = squeeze2d(fea1, 4)
        # m=mean1
        # mean2 = squeeze2d(fea2, 4)
        # m=torch.cat((m,mean2),dim=1)
        # mean3 = squeeze2d(fea3, 4) 
        # m=torch.cat((m,mean3),dim=1)
        # mean4 = squeeze2d(fea4, 4)
        # m=torch.cat((m,mean4),dim=1)
        # z=m
