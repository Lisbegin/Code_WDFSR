...
class RRDBNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, scale=4, opt=None):
        ...
        #self.conv_first_ = nn.Conv2d(in_nc+3, nf, 3, 1, 1, bias=True)
    ...
    def forward(self, x, get_steps=False):
        ...
        # 80    1
        fea_up2 = self.upconv1(F.interpolate(last_lr_fea, scale_factor=1/2, mode='nearest'))
        fea = self.lrelu(fea_up2)
        #40     2
        fea_up4 = self.upconv2(F.interpolate(fea, scale_factor=1/2, mode='nearest'))
        fea = self.lrelu(fea_up4)
        #20     3
        fea_up8 = self.upconv3(F.interpolate(fea, scale_factor=1/2, mode='nearest'))
