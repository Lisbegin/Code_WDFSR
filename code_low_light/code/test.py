...
def main():
    ...
    for lr_path, hr_path, idx_test in zip(lr_paths, hr_paths, range(len(lr_paths))):
        ...
        lr_t = t(lr)
        print(lr_t.shape)
        # print(lr_t.shape,hist_t.shape)
        # print(lr_t,hist_t)
        #lr_t=torch.cat((lr_t,hist_t),dim=1)
        ...
        path_out_sr = os.path.join(test_dir, "{:0.2f}".format(heat).replace('.', ''), "{:06d}_{:03}.png".format(idx_test,V))
        ...
        #meas['G_SSIM']=measure.measure(cv2.cvtColor(sr,cv2.COLOR_BGR2GRAY),cv2.cvtColor(hr,cv2.COLOR_BGR2GRAY))
