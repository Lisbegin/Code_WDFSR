...
class FlowUpsamplerNet(nn.Module):
    def __init__(self, image_shape, hidden_channels, K, L=None,
        ...
        self.K_2 = opt_get(opt, ['network_G', 'flow', 'K_2'])
        self.K_3 = opt_get(opt, ['network_G', 'flow', 'K_3'])
        ...
        if isinstance(self.K_2, int):
            self.K_2 = [K_2 for K_2 in [self.K_2, ] * (self.L + 1)]
        if isinstance(self.K_3, int):
            self.K_3 = [K_3 for K_3 in [self.K_3, ] * (self.L + 1)]
        self.opt = opt
        ...
        for level in range(1, self.L+1):
            ...
            if level==1:
                ...
                self.arch_FlowStep(H, self.K_2[level], LU_decomposed, W, actnorm_scale, affineInCh, flow_coupling,
                                flow_permutation,
                                hidden_channels, normOpt, opt, opt_get,
                                n_conditinal_channels=conditional_channels[level],type=1)
            elif level==3:
